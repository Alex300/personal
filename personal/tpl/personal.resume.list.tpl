...
<div class="row">
    ...
    <div class="col-xs-12 col-sm-9 col-sm-pull-3">
        ...
        <div class="well well-sm">
            <form id="searchForm" class="" method="get" action="{PHP|cot_url('personal', 'a=resume')}">
                <input type="hidden" id="searchTypeA" name="a" value="resume" />
                <div class="row">
                ...
                <div id="ext-search-form" style="display: none">
                    <div class="row">
                        <div class="col-xs-12 col-sm-6">
                            ...
                            <div class="row" style="margin-top: 5px">
                                <label for="" class="col-sm-4" style="line-height: 34px">{PHP.L.Gender}</label>
                                <div class="col-sm-8">{FILTER_GENDER}</div>
                            </div>

                            <div class="row" style="margin-top: 5px">
                                <label for="" class="col-sm-4" style="line-height: 34px">{PHP.L.Age}</label>
                                <div class="col-sm-8">
                                    <div class="row">
                                        <div class="col-xs-6 col-sm-2 text-right" style="line-height: 34px">{PHP.L.personal_from}</div>
                                        <div class="col-xs-6 col-sm-4">{FILTER_AGE_FROM}</div>
                                        <div class="col-xs-6 col-sm-2 text-right" style="line-height: 34px">{PHP.L.personal_to}</div>
                                        <div class="col-xs-6 col-sm-4">{FILTER_AGE_TO}</div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="col-xs-12 col-sm-9">
                    <header>
                        <h2 style="margin-top: 0">
                            <a href="{RESUME_ROW_URL}" title="{RESUME_ROW_TITLE}">{RESUME_ROW_TITLE}</a>
                        </h2>
                    </header>
                    <!-- IF {RESUME_ROW_CATEGORIES_COUNT} > 0 -->
                    {PHP.L.personal_category}:
                    <!-- BEGIN: CATEGORY_ROW -->
                    <!-- IF {RESUME_ROW_CATEGORY_ROW_NUM} > 1 -->,<!-- ENDIF -->
                    <a href="{RESUME_ROW_CATEGORY_ROW_ID|cot_url('personal', 'a=resume&f[cat]=$this')}">{RESUME_ROW_CATEGORY_ROW_TITLE}</a>
                    <!-- END: CATEGORY_ROW -->
                    <!-- ENDIF -->
                </div>

                <footer class="col-xs-12 col-sm-3 text-right">
                    <p><span class="label label-info">{RESUME_ROW_SORT_TEXT}</span></p>
                    <!-- IF {RESUME_ROW_CITY} > 0 --><p class="margin0">{RESUME_ROW_CITY_NAME}</p><!-- ENDIF -->
                    <p class="margin0">№ {RESUME_ROW_ID}</p>
                </footer>
            </div>
        </article>
        <!-- END: RESUME_ROW -->
